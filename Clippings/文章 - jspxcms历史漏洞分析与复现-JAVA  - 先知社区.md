...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220210213120-bb6d4166-8a75-1.png)  
修改application.properties数据库信息，最后启动Tomcat就部署完成了。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220210213544-58e5cb3e-8a76-1.png)
...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220210214753-0b64bca6-8a78-1.png)  
查看请求包可以看到请求路径是/comment\_submit，通过路径定位到源码。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220210215050-74ba5d1e-8a78-1.png)  
在IDEA使用Ctrl+Shift+F搜索comment\_submit，很容易就可以找到，在此处下断点进行调试。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211082231-b3a3c0bc-8ad0-1.png)  
重新发布一条评论，回到 IDEA，可以看到变量 text 接收了评论的内容，然后又调用了 submit。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211082554-2cbbe06a-8ad1-1.png)  
...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211083938-181800d8-8ad3-1.png)  
可以看到看到 text 的内容还是没有改变，跟到这里就行了，因为 dao 层一般只做跟数据库相关的操作，不会有任何安全处理。就这样评论的内容被写入到数据库了。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211084510-dda61772-8ad3-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211084826-52d12bea-8ad4-1.png)  
但是在文章页面并没有触发XSS，所以需要寻找是否有其他页面可以触发。在 burpsute 可以看到评论的内容在请求 /comment\_list 时得到，并且该内容进行了HTML实体编码。使用 IDEA 的搜索功能查找该前端页面。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211085551-5c1f6152-8ad5-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211085955-ed244e10-8ad5-1.png)  
直接到实际处理数据的 list 方法下断点进行调试。跟进 site.getTemplate 可以看到前端页面路径是 /1/default/sys\_comment\_list.html。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211090401-8030bf40-8ad6-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211090452-9e1f3496-8ad6-1.png)  
...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211092859-fc87a5ba-8ad9-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211093140-5cfeaff6-8ada-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211093313-93f604a0-8ada-1.png)  
...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211094326-01b3fc3a-8adc-1.png)  
查看如何访问 sys\_member\_space.html，可以看到文件名被定义为常量，space 方法使用了该常量，也就是说访问路径的格式为 /space/{id} 时就能触发 XSS 了。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211094911-cf657bf4-8adc-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211095310-5d96e02a-8add-1.png)  
XSS漏洞触发效果如下所示。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211095730-f8a96650-8add-1.png)
...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211153101-8ff74e40-8b0c-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211153826-99111d70-8b0d-1.png)  
在 fetchUrl() 调用了 fetchHtml()，并且这个方法可以直接被 HTTP 访问。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211153518-295c2b50-8b0d-1.png)  
使用 python 开启一个简易的 http 服务进行测试，测试效果如下所示，成功触发 SSRF。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211154427-7043e368-8b0e-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211154533-97aee02e-8b0e-1.png)

#### 第二处 SSRF：

搜索 openConnection ，可以看到在方法 ueditorCatchImage() 下，参数 source\[\] 直接可控，当执行到 conn.getContentType().indexOf("image") 时就会去请求相应的资源。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211161917-4e03a2f2-8b13-1.png)  
搜索调用 ueditorCatchImage() 方法的位置，可以看到访问路径为 /ueditor，action 参数需要等于 catchimage。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211162225-be7d3ca0-8b13-1.png)  
构造 payload 触发漏洞，如下所示成功触发 SSRF。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211162716-6be6d036-8b14-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211162732-7508e2f8-8b14-1.png)

### 三、RCE

#### 第一处 反序列化RCE

在审计 RCE 时需要先查看项目使用了哪些依赖包，可以看到项目中的依赖包符合 ysoserial 中的 CommonsBeanutils1 的条件，但是依赖包版本有一些差异。![](https://xzfile.aliyuncs.com/media/upload/picture/20220211190406-54d62f84-8b2a-1.png)  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211190758-df18c756-8b2a-1.png)  
我们直接在项目中创建一个 test 类进行测试，查看反序列化是否能成功执行，我在测试时发现反序列 ysoserial 的 CommonsBeanutils1 并不能成功，然后我使用之前跟p牛学的payload可以成功弹出计算器，如下图所示。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220211191831-5864bec0-8b2c-1.png)  
**Payload：**
...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220212082231-de1d0898-8b99-1.png)  
爆破出可以攻击的 rememberMe Cookie 大概需要一个多小时，如下界面所示。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220212082304-f21df834-8b99-1.png)  
...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220212101341-65bcce5a-8ba9-1.png)  
这里我使用冰蝎的 jsp webshell [冰蝎下载链接](https://github.com/rebeyond/Behinder "冰蝎下载链接")，将 webshell 打包成 war 包。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220212093622-2f678b42-8ba4-1.png)  
然后将 war 包打包成压缩文件。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220212094158-f7b50764-8ba4-1.png)  
注意：这里测试需要启动 tomcat 做测试，而不是 IDEA 的 SpringBoot，否则可能无法成功。  
上传完之后连接 webshell 成功 RCE。  
![](https://xzfile.aliyuncs.com/media/upload/picture/20220212101606-bc0dbe54-8ba9-1.png)  
...
![](https://xzfile.aliyuncs.com/media/upload/picture/20220212111338-c588b7f6-8bb1-1.png)  
...
- [![](https://xz.aliyun.com/api/v2/files/6e330c36-532c-34eb-a02a-1e1995893bc9)](https://xz.aliyun.com/news/16796)

[Merlin后渗透利用框架之Merlin Agent远控木马剖析](https://xz.aliyun.com/news/16796)
- [![](https://xz.aliyun.com/api/v2/files/e06f8787-c53f-3296-9249-074c601b175b)](https://xz.aliyun.com/news/16753)

[cyberstrikelab—Thunder](https://xz.aliyun.com/news/16753)
- [![](https://xz.aliyun.com/api/v2/files/f0f414a2-fa21-3101-8d75-1e5f8b9aa08e)](https://xz.aliyun.com/news/16779)

[内核攻防-(2)致盲EDR](https://xz.aliyun.com/news/16779)
- [![](https://xz.aliyun.com/api/v2/files/a376414a-735e-395e-b6c0-93b0adcec75d)](https://xz.aliyun.com/news/16765)
